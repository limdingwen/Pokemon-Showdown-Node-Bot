...
var parser = function (type, data) {
	...
	switch (type) {
		...
		case 'turn':
			...
			send(this.room, "/pm " + data.name + ", **" + exports.title + ":** " + trans("hand1", this.room) + " " + data.name + "" + trans("hand2", this.room) + ": " + formateHand(data.hand, this.getHandValue(data.hand), trans("total", this.room)));
			break;
		case 'player':
			if (data.type === "stand") {
